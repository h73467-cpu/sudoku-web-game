...
(function () {
  ...
  function hasAnyProgressToLose() {
    return FifteenGame.hasProgress() || FifteenGame.hasSavedResumableGame();
  }

  function applySoundButtonState(btn, compact) {
    const on = FifteenSound.isEnabled();
    btn.setAttribute("aria-pressed", on ? "true" : "false");
    btn.textContent = compact ? (on ? "🔊" : "🔇") : on ? "🔊 音效：開" : "🔇 音效：關";
  }

  function toggleSound() {
    const next = !FifteenSound.isEnabled();
    FifteenSound.setEnabled(next);
    FifteenStorage.saveSettings({ soundEnabled: next });
  ...
  // -- home view rendering ----------------------------------------------------
  function renderHome() {
    continueBtn.disabled = !FifteenGame.hasSavedResumableGame();
    const settings = FifteenStorage.getSettings();
  ...
  function renderHistory() {
    const items = FifteenStorage.getHistory();
    ...
    items.forEach((entry) => {
      ...
      row.innerHTML =
        ...
        `<span>用時 ${FifteenGame.formatSeconds(entry.elapsedSeconds)}　步數 ${entry.moves}</span>`;
  ...
  function renderCareer() {
    const career = FifteenStorage.getCareer();
    ...
    DIFFICULTY_ORDER.forEach((code) => {
      ...
      tr.innerHTML =
        ...
        `<td>${FifteenGame.formatSeconds(entry.bestTime)}</td>` +
  ...
  function renderBoard(state) {
    const size = FifteenGame.getBoardSize();
    boardEl.style.setProperty("--cols", size.cols);
    boardEl.style.setProperty("--rows", size.rows);
    const frag = document.createDocumentFragment();
    const movable = new Set();
    const row = Math.floor(state.blankIndex / size.cols);
    const col = state.blankIndex % size.cols;
    if (row > 0) movable.add(state.blankIndex - size.cols);
    if (row < size.rows - 1) movable.add(state.blankIndex + size.cols);
    if (col > 0) movable.add(state.blankIndex - 1);
    if (col < size.cols - 1) movable.add(state.blankIndex + 1);

    state.tiles.forEach((value, index) => {
      if (value === 0) {
        const blank = document.createElement("div");
        blank.className = "fifteen-blank";
        frag.appendChild(blank);
        return;
      }
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "fifteen-tile" + (movable.has(index) ? " movable" : "");
  ...
  function renderToolbar(state) {
    timerDisplay.textContent = FifteenGame.formatTime(FifteenGame.getElapsedMs());
    const career = FifteenStorage.getCareer();
  ...
  function renderWinModal(state) {
    ...
    winStats.innerHTML =
      statRow("花費時間", FifteenGame.formatTime(state.elapsedMs)) +
  ...
  function render(state, event) {
    if (SOUND_EVENTS.has(event)) FifteenSound.play(event);
    if (!state) return;
    if (event === "move" && state.status === "won") FifteenSound.play("win");
    if (event !== "tick") {
      renderBoard(state);
    }
    renderToolbar(state);
  ...
  continueBtn.addEventListener("click", () => {
    if (FifteenGame.resumeGame()) showView("game");
  });
  ...
  difficultyButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (difficulty === "superEasy") {
        ...
        FifteenStorage.saveSettings({ superEasyPercent: x });
      }
      FifteenGame.newGame(difficulty);
      showView("game");
    });
  });
  ...
  boardEl.addEventListener("click", (e) => {
    const btn = e.target.closest(".fifteen-tile");
    if (!btn || btn.disabled) return;
    FifteenGame.moveTile(Number(btn.dataset.index));
  });
  ...
  resetBtn.addEventListener("click", () => {
    const state = FifteenGame.getState();
    if (
      FifteenGame.hasProgress() &&
      !confirm("確定要放棄目前進度，重新開始一局新的關卡嗎？")
    ) {
      return;
    }
    FifteenGame.newGame(state ? state.difficulty : "superEasy");
  });

  backHomeBtn.addEventListener("click", () => {
    // No confirm needed: like klotski/sokoban, this autosaves after every
    // move, so "繼續上次遊戲" always brings it back.
    renderHome();
    showView("home");
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowUp") {
      e.preventDefault();
      FifteenGame.moveBlank(-1, 0);
    } else if (e.key === "ArrowDown") {
      e.preventDefault();
      FifteenGame.moveBlank(1, 0);
    } else if (e.key === "ArrowLeft") {
      e.preventDefault();
      FifteenGame.moveBlank(0, -1);
  ...
  // -- boot -----------------------------------------------------------------
  FifteenSound.setEnabled(FifteenStorage.getSettings().soundEnabled !== false);
  FifteenGame.onChange(render);
